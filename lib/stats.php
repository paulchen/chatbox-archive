...
function spammer_smiley($row) {
	if($row['top_spammer'] != '$$$$') {
		$parts = explode('$$', $row['top_spammer']);
		$row['top_spammer'] = "<a href=\"details.php?user={$parts[1]}\">{$parts[1]}</a> ({$parts[2]}x)";
	}
	else {
		$row['top_spammer'] = '-';
	}

	if($row['popular_smiley'] != '$$$$') {
		$parts = explode('$$', $row['popular_smiley']);
		$row['popular_smiley'] = "<a href=\"details.php?smiley={$parts[0]}\"><img src=\"images/smilies/{$parts[1]}\" alt=\"\" /></a> ({$parts[2]}x)";
	}
	else {
		$row['popular_smiley'] = '-';
	}
	...
	if($row['popular_word'] != '$$$$') {
		$parts = explode('$$', $row['popular_word']);
		$row['popular_word'] = "<a href=\"details.php?word=" . urlencode($parts[1]) . "\">{$parts[1]}</a> ({$parts[2]}x)";
	}
	else {
		$row['popular_word'] = '-';
	}
...
function smiley_column($row) {
	...
	$smiley_info = explode('$$', $row['smiley_info']);
	$id = $smiley_info[0];
	$filename = $smiley_info[1];
	$count = $smiley_info[2];

	$row['smiley_info'] = "<a href=\"details.php?smiley=$id\"><img src=\"images/smilies/$filename\" alt=\"\" /></a>&nbsp;(${count}x)";
...
function word_column($row) {
	...
	$word_info = explode('$$', $row['word_info']);
	$id = $word_info[0];
	$word = $word_info[1];
	$count = $word_info[2];
	$link = 'details.php?word=' . urlencode($word);
	$row['word_info'] = "<a href=\"$link\">$word</a>&nbsp;(${count}x)";
...
foreach($queries as $index => $query) {
	...
	if(isset($query['processing_function_all'])) {
		if(is_array($query['processing_function_all'])) {
			...
			$data = call_user_func($query['processing_function_all'], $data);
		}
	}
